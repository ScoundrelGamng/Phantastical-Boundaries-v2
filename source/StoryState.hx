...
class StoryState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			if(!isLocked || !weekFile.hiddenUntilUnlocked)
			{
				...
				if (isLocked)
				{
					var lock:FlxSprite = new FlxSprite(0);
					lock.frames = ui_tex;
		...
		//gap background
		var weekblockerbar:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('menuYKR'));
		...
		stageImage.antialiasing = ClientPrefs.globalAntialiasing;
		add(stageImage);
		...
		if(sprDifficulty.graphic != newImage)
		{
			...
			sprDifficulty.setGraphicSize(Std.int(sprDifficulty.width * 0.30));
			...
			sprDifficulty.y = FlxG.height - 200;
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.BACK && !movedBack && !selectedWeek)
		{
			...
			movedBack = true;
			MusicBeatState.switchState(new ModeState());
		}
	...
	function changeWeek(change:Int = 0):Void
	{
		...
		stageImage.loadGraphic(Paths.image('storymenu/stage' + (curWeek + 1)));
